...
//{$IFEND}
   ...
   StoreWindow;
    ...
    frmMain.Left := -5;  //此参数是窗口化时，内置网页不能显示正常
    frmMain.Top := -29;  //此参数是窗口化时，内置网页不能显示正常
    end;
...
begin
   if mrOk = FrmDlg.DMessageDlg ('你想退出到选择角色界面吗？', [mbOk, mbCancel]) then begin
...
begin
   if mrOk = FrmDlg.DMessageDlg ('你想退出传奇吗?', [mbOk, mbCancel]) then begin
      if g_boBagLoaded then
         Savebags ('.\Data\' + g_sServerName + '.' + CharName + '.itm', @g_ItemArr);
      g_boBagLoaded := FALSE;
      OutClmainall;
         ...
         begin
            ...
            if ssAlt in Shift then begin
               ...
               begin
                  ...
                  DScreen.AddChatBoardString ('战斗状态中不能退出游戏...', clYellow, clRed);
            end;
         end;
      word('Q'):
         begin
...
begin
   ...
   if (abs(g_MySelf.m_nCurrX - target.m_nCurrX) <= 1) and (abs(g_MySelf.m_nCurrY-target.m_nCurrY) <= 1) and (not target.m_boDeath) then begin
      if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
         ...
         if g_boCanLongHit
          and (TargetInSwordLongAttackRange (tdir)) Or( g_WgInfo.boCanLongHit)then begin  //氛 酒咆牢 版快, 绢八贱
            nHitMsg := CM_LONGHIT;
         end;
         ...
         if not g_TargetCret.m_boDeath and PlayScene.IsValidActor(g_TargetCret) then begin
            ...
            if GetKeyboardState (keyvalue) then begin
               ...
               if ((g_TargetCret.m_btRace <> RCC_USERHUMAN) and
                 ...
                 // or ((ssShift in Shift) Or g_boShift) and (not PlayScene.EdChat.Visible)) then begin //荤恩阑 角荐肺 傍拜窍绰 巴阑 阜澜
...
begin
   if Length(rstr) >= 2 then begin  //颇扼皋鸥啊 鞘夸茄 版快啊 乐澜.
      if (rstr[1] = '@') and (rstr[2] = '@') then begin
         ...
         else FrmDlg.DMessageDlg ('请输入。', [mbOk, mbAbort]);
...
begin
  If (g_MySelf.IsIdle) And
    ((g_MySelf.m_nState And $04000000 = 0) Or (g_wgInfo.boParalyCan )) And //防石化
      ...
      case msg.Ident of
         ...
         SM_NEWID_FAIL:
            begin
               case msg.Recog of
                  ...
                  -2: FrmDlg.DMessageDlg('在本服务器上，此用户名已被禁止使用。\请另选一个不同的名字。', [mbOk]);
                  else
         ...
         SM_PASSWD_FAIL:
            ...
            Case msg.Recog Of
              ...
              -2: FrmDlg.DMessageDlg('[失败]：账号被锁定，密码错误超过三次，请稍后再试！', [mbOk]);
              -3: FrmDlg.DMessageDlg('[失败]：账号被锁定，可能用户正在使用当中！', [mbOk]);
              -4: FrmDlg.DMessageDlg('[失败]：请重新注册您的账号！',[mbOk]);
              -5: FrmDlg.DMessageDlg('[失败]：您的账号已过期，已被暂停使用！', [mbOk]);
            Else
         ...
         SM_NEWCHR_FAIL: begin
              Case msg.Recog Of
              0: FrmDlg.DMessageDlg('[失败]：角色创建失败，\ \角色名中只允许数字、英文和简体汉字字符.\请确认角色名中是否带有非法字符', [mbOk]);
              2: FrmDlg.DMessageDlg('[失败]：这个名字已存在.', [mbOk]);
              3: FrmDlg.DMessageDlg('[失败]：你最多只能为一个账号设置2个角色.', [mbOk]);
              4: FrmDlg.DMessageDlg('[失败]：角色创建失败，\ \角色名中只允许数字、英文和简体汉字字符.\请确认角色名中是否带有非法字符', [mbOk]);
              5:FrmDlg.DMessageDlg('[失败]：你所创建的角色总数已达上限.', [mbOk]);
            Else
         ...
         SM_CHGPASSWD_FAIL: begin
              Case msg.Recog Of
              -1: FrmDlg.DMessageDlg('[失败]：原密码或账号错误，不能修改.', [mbOk]);
              -2:FrmDlg.DMessageDlg('[失败]：账号被锁定，请稍后再试.',[mbOk]);
            Else
...
//               frmSelMain.Close;
         ...
         SM_VERSION_FAIL: begin
           FrmDlg.DMessageDlg('版本错误. 请重新下载客户端. (http://www.legendofmir.net)', [mbOk]);
           FrmMain.Close;
           exit;
         end;
         SM_OUTOFCONNECTION,
...
//        PlayScene.MemoLog.Lines.Add('X: ' + IntToStr(msg.Param) + 'Y: ' + IntToStr(msg.tag) + ' Map: ' + str);
        ...
        SM_OpenItemArray:   //2015-06-07 解包列表
         begin
      ...
      SM_WINEXP:
         begin
            ...
            DScreen.AddChatBoardString (IntToStr(LongWord(MakeLong(msg.Param,msg.Tag))) + ' 点经验值增加.',clWhite, clRed);
         end;
...
//            if WaitingUseItem.Index in [0..8] then
      ...
      SM_USERSELLITEM_FAIL:
         begin
            ...
            g_SellDlgItemSellWait.S.Name := '';
             Case Msg.Recog Of
            -1: FrmDlg.DMessageDlg(Format('输入的价格不能小于1或大于%d。', [High(Integer)]), [mbOk]);
      ...
      SM_BUYITEM_FAIL:
         begin
            ...
            2: FrmDlg.DMessageDlg('没有更多的物品可以携带了.',  [mbOk]);
      ...
      SM_CREATEGROUP_FAIL:
         begin
            ...
            case msg.Recog of
              ...
              -2:FrmDlg.DMessageDlg('这个被加进编组的名字是不正确的.', [mbOk]);
              -3: FrmDlg.DMessageDlg('你想加进编组的这位用户已经是其它编组的成员了.', [mbOk]);
      ...
      SM_READMINIMAP_FAIL: begin
        g_dwQueryMsgTick := GetTickCount;
        DScreen.AddChatBoardString ('没有可用地图。', clWhite, clRed);
        g_nMiniMapIndex:= -1;
      end;
      ...
      SM_GUILDADDMEMBER_FAIL: begin
        case msg.Recog of
              1: FrmDlg.DMessageDlg('你没有权力使用这个命令.', [mbOk]);
              2: FrmDlg.DMessageDlg('想加入进来的成员应该面对掌门人.', [mbOk]);
              3: FrmDlg.DMessageDlg('他已加入我们行会.', [mbOk]);
              4: FrmDlg.DMessageDlg('已经加入其它行会.', [mbOk]);
              5: FrmDlg.DMessageDlg('对方不允许加入行会.', [mbOk]);
              6: FrmDlg.DMessageDlg('你的行会成员人数已达最高上限.', [mbOk]);
        end;
      end;
      ...
      SM_GUILDDELMEMBER_FAIL: begin
        case msg.Recog of
              ...
              3: FrmDlg.DMessageDlg('行会掌门人不能删除他自己！', [mbOk]);
              4: FrmDlg.DMessageDlg('不能使用命令.', [mbOk]);
        end;
      end;
      SM_GUILDRANKUPDATE_FAIL: begin
        case msg.Recog of
              -2: FrmDlg.DMessageDlg('[任命错误] 掌门人位置是空的。',[mbOk]);
              -3: FrmDlg.DMessageDlg('[任命错误] 掌门人职位是空的。',[mbOk]);
              -4: FrmDlg.DMessageDlg('[任命错误] 一个行会最多只能有2个掌门人。', [mbOk]);
              -5: FrmDlg.DMessageDlg('[任命错误] 新的掌门人已经被传位。', [mbOk]);
              -6: FrmDlg.DMessageDlg('[任命错误] 不能添加成员/删除成员。', [mbOk]);
              -7: FrmDlg.DMessageDlg('[任命错误] 职位重复或者出错。',[mbOk]);
        end;
      end;
      SM_GUILDMAKEALLY_OK,
      SM_GUILDMAKEALLY_FAIL: begin
        case msg.Recog of
              ...
              -3: FrmDlg.DMessageDlg('你应该面对你想要联盟的行会掌门人！', [mbOk]);
      ...
      SM_GUILDBREAKALLY_FAIL: begin
        case msg.Recog of
              ...
              -3: FrmDlg.DMessageDlg('这不是一个现有的行会！', [mbOk]);
        end;
      end;
...
procedure TfrmMain.HintBoss(actor:Tactor);      //2015-06-07  添加BOSS提示
var
  i:integer;
  ndir:Integer;
  sDir:String;
  ...
  try
    ...
    begin
      ...
      if PlayEatItem(g_wgInfo.boAutoCropsHpName, 3) = -1 then
        DScreen.AddChatBoardString(Format(sTest, ['特殊药品']), clWhite, clRed);
    end;
  except
  end;
    //自动补红
  try
    ...
    begin
      ...
      if PlayEatItem(g_wgInfo.boAutoHpName, 1) = -1 then
        DScreen.AddChatBoardString(Format(sTest, ['普通HP类药品']), clWhite, clRed);
    end;
  except
  end;
    //自动补蓝
  try
    ...
    begin
      ...
      if PlayEatItem(g_wgInfo.boAutoMpName, 2) = -1 then
        DScreen.AddChatBoardString(Format(sTest, ['普通MP类药品']), clWhite, clRed);
    end;
  except
  end;
