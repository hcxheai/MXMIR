...
uses
   ClMain, MShare, Share, SDK, NeiGua;
...
begin
   ...
   if d <> nil then begin
      DLOGO.SetImgIndex (g_WMain99Images, 103);
      DLOGO.Left := (SCREENWIDTH - d.Width) div 2;
      DLOGO.Top := (SCREENHEIGHT - d.Height) div 2;
   end;
   DLOGO.Left := 0;
   DLOGO.Top := 0;
   DLOGO.Width := 800;
      ...
      case StatePage of
         0: begin //自己装备
            ...
            if g_MySelf <> nil then
               if g_MySelf.m_btSex = 1 then pgidx := 30{377};  //女4格
            bbx := Left + 38;
            bby := Top + 52;
      ...
      if g_MouseStateItem.S.Name <> '' then begin
         ...
         if iname <> '' then begin
            ...
            with dsurface.Canvas do begin
               ...
               Font.Size := 9;
               Font.Color := clYellow;
               TextOut (SurfaceX(Left+37), SurfaceY(Top+310), iname);
               Font.Color := hcolor;
               TextOut (SurfaceX(Left+37+TextWidth(iname)), SurfaceY(Top+310), d1);
               TextOut (SurfaceX(Left+37), SurfaceY(Top+310+TextHeight('A')+2), d2);
               TextOut (SurfaceX(Left+37), SurfaceY(Top+310+(TextHeight('A')+2)*2), d3);
               Font.Size := old;
               Release;
            end;
         end;
   ...
   if g_MouseItem.S.Name <> '' then begin
      ...
      case g_MouseItem.S.StdMode of
         ...
         5..6: //武器
            begin
               ...
               if g_MouseItem.S.SC > 0 then
                  line2 := line2 + '道' + IntToStr(LoWord(g_MouseItem.S.SC)) + '-' + IntToStr(HiWord(g_MouseItem.S.SC)) + ' ';
                  ...
                  21,54:  //项链
                     begin
                        ...
                        if HiWord(g_MouseItem.S.MAC) > 0 then
                           line2 := line2 + '魔法恢+' + IntToStr(HiWord(g_MouseItem.S.MAC)) + '0% ';
                        if LoWord(g_MouseItem.S.AC) > 0 then
         ...
         25: //护身符及毒药
            begin
               line1 := line1 + sWgt +  IntToStr(g_MouseItem.S.Weight);
               line2 := '使用 '+ GetDura100Str(g_MouseItem.Dura, g_MouseItem.DuraMax);
            end;
         30: //照明物
            begin
         ...
         42: //药材
            begin
               line1 := line1 + sWgt +  IntToStr(g_MouseItem.S.Weight) + ' 距犁';
            end;
         43: //矿石
            begin
...
begin
   with dsurface.Canvas do begin
      ...
      if not BoStorageMenu then begin
        ...
        TextOut(SX(19), SY(11), '托管物品列表' + Format(' (%d件)', [MenuList.Count]));
         ...
         if (g_MovingItem.Index = -97) or (g_MovingItem.Index = -98) then begin //捣父..
            if (g_MovingItem.Index = -98) then begin //啊规芒俊辑 柯 捣
               if g_MovingItem.Item.S.Name = g_sGoldName{'金币'} then begin
                  ...
                  g_boItemMoving := FALSE;
                  g_MovingItem.Item.S.Name := '';
                  DMessageDlg('你想支付多少'+ g_sGoldName + '?',[mbOk, mbAbort]);
                  GetValidStrVal (DlgEditText, valstr, [' ']);
   ...
   with DGuildEditNotice do begin
      ...
      if DMsgDlg.DialogResult = mrOk then begin
         ...
         if Length(data) > 4000 then begin
            ...
            DMessageDlg('由于句子太长，最后的部分被移除了.',[mbOk]);
         end;
...
begin
   if GuildMembers.Count <= 0 then begin
      DMessageDlg ('请点击[名单]按扭以调出行会成员列表.', [mbOk]);
      exit;
   end;
...
begin
   GuildEditHint := '[修改行会成员的等级和职位。#警告：不能 增加/删除 行会成员。]';
   ShowGuildEditGrade;
end;

procedure TFrmDlg.DGDAllyClick(Sender: TObject; X, Y: Integer);
begin
   if mrOk = DMessageDlg ('和对方行会结盟应该在 [允许结盟] 的状态下。\' +
                  '而且你应该面对对方行会首领。\' +
                  '你想结盟吗？', [mbOk, mbCancel])
   then
      FrmMain.SendSay ('@联盟');
...
begin
   DMessageDlg ('请输入你想取消结盟的行会的名字。', [mbOk, mbAbort]);
   if DlgEditText <> '' then
...
begin
  Butt:=TDButton(Sender);
  if Sender = DBotMiniMap then sMsg:= '小地图';
  if Sender = DBotTrade then sMsg:= '物品交易';
  if Sender = DBotGuild then sMsg:= '行会信息';
  if Sender = DBotGroup then sMsg:= '组队控制';
  if Sender = DBotPlusAbil then sMsg:= '属性';
 // if Sender = DBotFriend then sMsg:= '关系系统';
  if Sender = DBotLogout then sMsg:= '小退(Alt-X)';
  if Sender = DBotExit then sMsg:= '退出(Alt-Q)';

  if Sender = DMyState then sMsg:= '状态信息';
  if Sender = DMyBag then sMsg:= '背包信息';
  if Sender = DMyMagic then sMsg:= '技能信息';
...
begin
  with Sender as TDWindow do begin
    ...
    SetBkMode (dsurface.Canvas.Handle, TRANSPARENT);
    BoldTextOut (dsurface, SurfaceX(Left+15), SurfaceY(Top+13), clWhite, clBlack, g_sGamePointName);

    dsurface.Canvas.Font.Style := dsurface.Canvas.Font.Style + [fsUnderline];
    BoldTextOut (dsurface, SurfaceX(Left+12), SurfaceY(Top+190), clYellow, clBlack, g_sGameGoldName);
    dsurface.Canvas.Font.Style := dsurface.Canvas.Font.Style - [fsUnderline];
    dsurface.Canvas.Release;
  end;
end;
...
procedure TFrmDlg.NewWgWindows();   //打开内挂
var
i:integer;
d: TDirectDrawSurface;
begin

   if not Assigned(frmNeiGua) then
    begin
      frmNeiGua := TfrmNeiGua.Create(FrmMain);
      frmNeiGua.Open;
      frmNeiGua.ParentWindow := FrmMain.Handle;
      frmNeiGua.Show;
      frmNeiGua.RefWgBut;
     end
  else
   frmNeiGua.Visible := not frmNeiGua.Visible;

end;

procedure TFrmDlg.DLOGOClick(Sender: TObject; X, Y: Integer);  //开始LOGO 版权绘制
begin
  DLOGO.Visible := False;
...
procedure TFrmDlg.DLOGODirectPaint(Sender: TObject;    //开始LOGO 版权绘制
  dsurface: TDirectDrawSurface);                       
var
  d: TDirectDrawSurface;
begin
