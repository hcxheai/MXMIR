...
interface
uses
  Windows, Classes, SysUtils, cliutil, Forms, DXDraws, DWinCtl, NWGfig,
  WIL, Actor, Grobal2, SDK, DXSounds, IniFiles, Share, GShare, ImageHlp;
...
var
 ...
 // g_WWeaponImages    :TWMImages;
  ...
  g_WMonImagesArr    :array[0..9999] of TWMImages;
    // 武器相关
  g_WWeaponImages: array[0..10] of TWMImages; { TODO : --- piaoyun 武器图片读取改为数组方式 2013-3-21 --- }
  // 衣服相关
  g_WHumImgImages: array[0..5] of TWMImages; { TODO : --- piaoyun 衣服图片读取改为数组方式 2013-3-22 --- }
...
//  g_WWeaponImages    :array of TWMImages;
  ...
  g_dwLogoTick              :LongWord; //版权广告显示时间
  g_nLogoTimer              :Byte;
 ...
 // g_boForceNotViewFog       :Boolean = True;//False;  //免蜡烛
...
// g_boBossHint           :Boolean  = False;  //Boss提示
...
procedure InitWeaponImg();
procedure InitHumImg();
...
{$IFEND}
  ...
  FillChar(g_WWeaponImages, SizeOf(g_WWeaponImages), 0);
  FillChar(g_WHumImgImages, SizeOf(g_WHumImgImages), 0);
...
begin
  ...
  g_WMain99Images.FileName := GAMRPIANIMAGEFILE;
...
begin
  ...
  for I := Low(g_WWeaponImages) to High(g_WWeaponImages) do begin
    if g_WWeaponImages[I] <> nil then begin
      g_WWeaponImages[I].Finalize;
      g_WWeaponImages[I].Free;
    end;
  end;


  for I := Low(g_WHumImgImages) to High(g_WHumImgImages) do begin
    if g_WHumImgImages[I] <> nil then begin
      g_WHumImgImages[I].Finalize;
      g_WHumImgImages[I].Free;
    end;
  end;
...
begin
  for I := Low(g_WWeaponImages) to (High(g_WWeaponImages)) do begin
    if I = 0 then
      ...
      sFileName := Format(WEAPONIMAGESFILE1, [I + 1]);
    g_WWeaponImages[i] := TWMImages.Create(nil);
    g_WWeaponImages[I].DxDraw := g_DxDraw;
    g_WWeaponImages[I].DDraw := g_DxDraw.DDraw;
    g_WWeaponImages[I].FileName := sFileName; ;
    g_WWeaponImages[I].LibType := ltUseCache;
    g_WWeaponImages[I].Initialize;
  end;
end;
...
begin
  for I := Low(g_WHumImgImages) to (High(g_WHumImgImages)) do begin
    if I = 0 then
      ...
      sFileName := Format(HUMIMGIMAGESFILE1, [I + 1]);
    g_WHumImgImages[i] := TWMImages.Create(nil);
    g_WHumImgImages[I].DxDraw := g_DxDraw;
    g_WHumImgImages[I].DDraw := g_DxDraw.DDraw;
    g_WHumImgImages[I].FileName := sFileName; ;
    g_WHumImgImages[I].LibType := ltUseCache;
    g_WHumImgImages[I].Initialize;
  end;
end;
